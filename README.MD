...
```cpp
// Enable display optimization (saves CPU by only updating changed pixels)
// NOTE: Automatically disabled for displays requiring >150KB RAM to prevent memory issues
...
void setOptimizeDisplayUpdates(bool enabled)   // Enable frame comparison (auto-disabled if >150KB RAM needed)
...
void showBrightnessControl() {
  // NOTE: When using String concatenation with PopupConfig, the String object
  // must remain valid while the popup is displayed. Using static ensures this.
  // Alternative: call PopupManager::showInfo(message.c_str(), "Title") directly
  // where the message String is a local variable (non-blocking popup).
  static String message;
  message = "Current brightness: " + String(brightness) + "%\nAdjust in Settings menu";
  
  PopupConfig config;
  config.message = message.c_str();
...
- **Flickering**: Enable display optimization with `setOptimizeDisplayUpdates(true)` (for small displays)
- **Corrupted display on large screens**: Library automatically disables optimization when >150KB RAM required
...
#### **Memory Issues**
- **Crashes on large displays (>172px wide)**: The library automatically disables `OptimizeDisplayUpdates` for displays requiring >150KB of RAM to prevent crashes
- **Heap overflow on ESP8266**: Disable display optimization and reduce menu complexity
- **Out of memory errors**: Reduce number of menu items or use simpler graphics
...
### **Memory Optimization**
- **Large displays (>150KB RAM needed)**: Library automatically disables `OptimizeDisplayUpdates` to prevent memory issues
- **Small displays (≤158×158 or equivalent area)**: Safe to enable `setOptimizeDisplayUpdates(true)` for CPU savings
- Use `setOptimizeDisplayUpdates(false)` on ESP8266 or when low on memory
- Minimize the number of concurrent menu items
- Use simple icons (16x16 pixels or smaller)
- Avoid deeply nested menu structures

### **CPU Optimization**
- Enable frame comparison on small displays: `setOptimizeDisplayUpdates(true)` (when <150KB RAM needed)
